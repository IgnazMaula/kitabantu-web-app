...
export default function ProviderManageOrder() {
    ...
    useEffect(() => {
        const getUsers = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}api/orders/provider/${auth.loggedUser.id}`);
            ...
            <div>
                <div className='max-w-7xl mx-auto sm:px-2 lg:px-8'>
                    <div className='max-w-2xl mx-auto px-4 lg:max-w-4xl lg:px-0'>
                        <h1 className='text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl'>Manage Incoming Order</h1>
                        <p className='mt-2 text-sm text-gray-500'>manage all service ordered.</p>
                    </div>
                </div>
